 ...
 *
 *  This method is to register a callback function that is triggered
...
{
	// the yoffset is adjusting movement speed in camera.h Ln:125-133
...
void ViewManager::ProcessKeyboardEvents()
{
	...
	// change between orthographic projection and perspective projection

	// press the O key to change it to orthographic
	if (glfwGetKey(m_pWindow, GLFW_KEY_O) == GLFW_PRESS)
	{
		// change to orthographic projection
		bOrthographicProjection = true;

		// change the camera settings to show orthographic view
		g_pCamera->Position = glm::vec3(6.0f, 9.13f, 5.0f);
		g_pCamera->Up = glm::vec3(0.0f,2.0f, 0.0f);
		g_pCamera->Front = glm::vec3(0.0f, 0.0f, -1.0f);
		g_pCamera->Zoom = 100;
	}
	// press the P key to change it to perspective
